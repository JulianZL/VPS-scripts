...
#change ssh port
sed -i -e "/Port /c\Port 100xx" /etc/ssh/sshd_config
echo -e "Restarting SSH in 5 seconds. Please wait.\n"
sleep 5
echo ""
echo -e "The SSH port has been changed to 100xx. Please login using that port to test.\n"

#update SELinux
echo -e "Update SELinux for port change.\n"
semanage port -a -t ssh_port_t -p tcp 100xx
sleep 5
